...
const FloatingWhatsApp = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end">
      {isOpen && (
        <div className="mb-4 bg-white rounded-2xl shadow-2xl p-5 w-80 border border-gray-100 transform origin-bottom-right transition-all duration-300 scale-100 opacity-100 relative overflow-hidden animate-fade-in">
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-gray-800 text-sm">Parça Sorgulama</h3>
            </div>
            <button
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
             <div className="relative">
              <input
                ...
                className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-100 focus:border-[#25D366] transition-all bg-gray-50 uppercase"
                required
              />
            </div>
            <button
      ...
      <div 
        className="group relative flex items-center justify-start bg-white rounded-full shadow-2xl transition-all duration-300 ease-in-out cursor-pointer hover:pr-5 border border-green-50 z-10 w-[60px] h-[60px] md:w-[64px] md:h-[64px] hover:w-[260px] md:hover:w-[280px]"
        style={{ boxShadow: '0 10px 40px rgba(37, 211, 102, 0.3)' }}
        onClick={() => setIsOpen(!isOpen)}
      >
        {/* Yuvarlak Logo Kısmı */}
        <div className="absolute left-0 top-0 flex items-center justify-center w-[60px] h-[60px] md:w-[64px] md:h-[64px] bg-white rounded-full z-20 transition-transform duration-300">
        ...
        <div className="absolute left-[64px] flex flex-col items-start whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 overflow-hidden">
          <span className="text-[14px] md:text-[15px] font-bold text-gray-800 leading-tight">Şasi Numaranızı Gönderin</span>
